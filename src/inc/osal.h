...
#ifdef LOG_TAG
#define __E(fmt, args...) ({ printk(KERN_ERR "E/%10s: %-10s/%3d: %s(): " fmt, __this_module.name, LOG_TAG, __LINE__, __FUNCTION__, ## args);})
#define __W(fmt, args...) ({ printk(KERN_ERR "W/%10s: %-10s/%3d: %s(): " fmt, __this_module.name, LOG_TAG, __LINE__, __FUNCTION__, ## args);})
#define __I(fmt, args...) ({ printk(KERN_ERR "I/%10s: %-10s/%3d: %s(): " fmt, __this_module.name, LOG_TAG, __LINE__, __FUNCTION__, ## args);})

#ifdef __DEBUG
#define __D(fmt, args...) ({ printk(KERN_ERR "D/%10s: %-10s/%3d: %s(): " fmt, __this_module.name, LOG_TAG, __LINE__, __FUNCTION__, ## args);})
#else
...
#define __E(fmt, args...)  ({fprintf(stderr, "E/%-10s: %20s/%3d: %s(): " fmt, MODULE_NAME, LOG_TAG,  __LINE__, __FUNCTION__, ## args);})
#define __W(fmt, args...)  ({fprintf(stderr, "W/%-10s: %20s/%3d: %s(): " fmt, MODULE_NAME, LOG_TAG,  __LINE__, __FUNCTION__, ## args);})
#define __I(fmt, args...)  ({fprintf(stderr, "I/%-10s: %20s/%3d: %s(): " fmt, MODULE_NAME, LOG_TAG,  __LINE__, __FUNCTION__, ## args);})

#ifdef NDEBUG
#define __D(fmt, args...)
#else
#define __D(fmt, args...)  ({fprintf(stderr, "D/%-10s  : %20s/%3d: %s(): " fmt, MODULE_NAME, LOG_TAG,  __LINE__, __FUNCTION__, ## args);})
#endif
...
typedef struct timer_list {
	uint32_t        sec;
	uint32_t        expire; /* expire = jiffies + sec * HZ */
	void     (*function)(unsigned long p);
	timer_t    timerid;
	void*      data;
} timer_list_t;

static timer_list_t *timer_list[8];

static void timer_function(int a, siginfo_t* b, void* c)
{
	int i;
	struct timer_list *timer = NULL;
	
	for(i = 0; i < ARRAY_SIZE(timer_list); i++) {
		if(timer_list[i]) {
			timer = timer_list[i];
			timer_delete(timer->timerid);
			timer_list[i] = NULL;
			
			timer->function((uint32_t)timer->data);
			return;
		}
	}
}
...
static inline void add_timer(struct timer_list *timer)
{
	...
	for(i = 0; i < ARRAY_SIZE(timer_list); i++) {
		if(timer_list[i] == NULL) {
			...
			act.sa_sigaction = timer_function;
			sigemptyset(&act.sa_mask);
			sigaction(SIGALRM, &act, NULL);

			timer_create(CLOCK_REALTIME, NULL, &timer->timerid);
			value.it_interval.tv_sec  = timer->sec;
			value.it_interval.tv_nsec = 0;
			value.it_value = value.it_interval;
			timer_settime(timer->timerid, 0, &value, NULL);
			return;
		}
	}
}

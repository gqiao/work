...
typedef struct tagS3C6400_MFC_PARAM_REG_FIRMWARE_VER {
	...
	// (0x180 ~ 0x1A0)
	unsigned int reserved1[16];		// 0x180 ~ 0x1bc

	//  OUTPUT RETURN 
	// (0x1C0 ~ 0x1D4)
	/* 0x1C0 */
	v32 RET_GET_FW_VER;  // Returned Version Code with following format: [31:16]: Product No (0xF202), [15:0]: Ver. No. (0xMmrr) for M.m.rr
...
typedef struct tagS3C6400_MFC_PARAM_REG_ENC_HEADER {
	...
	// (0x180 ~ 0x194)
	/* 0x180 */
	v32 ENC_HEADER_CODE;      // Encode header code

	/* 0x184 */
	v32 ENC_HEADER_BB_START;  // Byte address of the encoder output header syntax buffer. This value will only be valid if encoder dynamic buffer allocation option is enabled as well as stream buffer reset option

	/* 0x188 */
	v32 ENC_HEADER_BB_SIZE;   // Byte size of encoded header stream buffer. This value will only be valid if encoder dynamic buffer allocation option is enabled as well as stream buffer reset option

	v32 ENC_HEADER_NUM;  // 0x18c

	v32 reserved1[12];  // 0x190 ~ 0x1bc
...
typedef struct tagS3C6400_MFC_PARAM_REG_ENC_PARA_SET {
	...
	// (0x180 ~ 0x194)
	/* 0x180 */
	v32 ENC_PARA_SET_TYPE;  // Parameter set type. 0: Sequence Parameter Set, 1: Picture Parameter Set
...
typedef struct tagS3C6400_MFC_PARAM_REG_ENC_PIC_RUN {
	...
	// (0x180 ~ 0x194)
	/* 0x180 */
	v32 ENC_PIC_SRC_ADDR_Y;  // Encoding source frame address of luminance. Host must set SDRAM frame buffer start address to this register before every encoding picture
	
	/* 0x184 */
	v32 ENC_PIC_SRC_ADDR_CB;  // Encoding source frame address of Cb

	/* 0x188 */
	v32 ENC_PIC_SRC_ADDR_CR;  // Encoding source frame address of Cr
	
	/* 0x18C */
	v32 ENC_PIC_QS;  // Picture quantized step parameter for encoding process

	/* 0x190 */
	struct {
		v32 PreRotMode         :4,  // [3:0] Pre-rotation mode. PreRotMode[3:0] = {HorMir, VerMir, RotAng[1:0]}, HorMir: Horizontal mirroring, VerMir: Vertical mirroring RotAng[1:0].
			PreRotEn           :1;  // [4]   Pre-rotation enable. 1: the source image is rotated prior to encoding, 0: the pre-rotation is disabled and PreRotMode field is ignored
	} ENC_PIC_ROT_MODE;

	/* 0x194 */
	struct {
		v32 PicSkipEn          :1,  // Picture skip flag. 1: EncSrcAddrY, EncSrcAddrCb, EncSrcAddrCr are ignored and one skipped picture is encoded
			IdrPic             :1;  // 1: the source image is encoded as IDR(Instantaneous Decoding Refresh) picture at H.264 or I(Intra) picture at MPEG-4/H.263 regardless of [EncGopNum] value
	} ENC_PIC_OPTION;

	/* 0x198 */
	v32 ENC_PIC_BB_START;  // Byte address of the encoder output picture stream buffer. This value will only be valid if encoder dynamic buffer allocation option is enabled as well as stream buffer reset option

	/* 0x19C */
	v32 ENC_PIC_BB_SIZE;  // Byte size of encoded picture stream buffer. This value will only be valid if encoder dynamic buffer allocation option is enabled as well as stream buffer reset option
	
	v32 reserved[8]; // 0x1a0 ~ 0x1bc
	...
	/* 0x1C4 */
	v32 RET_ENC_PIC_TYPE;  // The picture type of current encoded picture. 0: I (Intra) picture, 1: P (Inter) pictur
	...
	/* 0x1CC */
	v32 RET_ENC_PIC_SLICE_NUM;  // If [SliceInfoReport] flag in CMD_ENC_SEQ_OPTION register, BIT returns encoded slice number to this register. The encoded slice end position of every slice is stored to SDRAM.

	/* 0x1D0 */
	v32 RET_ENC_PIC_FLAG;  // This bit is set if BitstreamBuffer wrptr is wrap arounded in reset mode. If this bit set, bitstream generated might be crashed
...
typedef struct tagS3C6400_MFC_PARAM_REG_DEC_PIC_RUN {
	...
	/* 0x180 */
	struct {
		v32 PostRotMode     :4,  // [3:0] Post rotation mode. PostRotMode[3:0] = {HorMir, VerMir, RotAng[1:0]}.
			PostRotEn       :1;  // [4] Post rotation enable.
	} DEC_PIC_ROT_MODE; 

	/* 0x184 */
	v32 DEC_PIC_ROT_ADDR_Y;  // Rotated display frame address of luminance

	/* 0x188 */
	v32 DEC_PIC_ROT_ADDR_CB;  // Rotated display frame address of Cb
	
	/* 0x18C */
	v32 DEC_PIC_ROT_ADDR_CR;  // Rotated display frame address of Cr

	/* 0x190 */
	v32 DEC_PIC_DBK_ADDR_Y;  // Deblocked display frame address of luminance

	/* 0x194 */
	v32 DEC_PIC_DBK_ADDR_CB;  // Deblocked display frame address of Cb DEC

	/* 0x198 */
	v32 DEC_PIC_DBK_ADDR_CR;  // Deblocked display frame address of Cr DEC

    /* 0x19C */
	v32 DEC_PIC_ROT_STRIDE;  // Rotated display frame Stride

	v32 DEC_PIC_OPTION;	     // 0x1a0, Decoding option
	
	v32 reserved1;			 // 0x1a4
	
	/* 0x1A8 */
	v32 DEC_PIC_CHUNK_SIZE;  // Byte size of picture stream data. This value will be only valid in file-play mode, and it will be used as size of stream data when BIT processor update write pointer of picture stream buffer.
	
	/* 0x1AC */
	v32 DEC_PIC_BB_START;  // 4-byte aligned byte address of the decoder input picture stream buffer. This value will only be valid if decoder dynamic buffer allocation option is enabled as well as file-play mode option
	
	/* 0x1B0 */
	v32 DEC_PIC_START_BYTE;  // Byte Address of valid bitstream in input picture stream buffer

	v32 DEC_PIC_MV_ADDR;		// 0x1b4, Base address for Motion Vector data
	v32 DEC_PIC_MBTYPE_ADDR;	// 0x1b8, Base address for MBType data
	...
	/* 0x1DC */
	v32 RET_DEC_PIC_CUR_IDX;  // Decoded frame index. After BIT decodes one frame, BIT return decoded frame index to this register

	v32 RET_DEC_PIC_FCODE_FWD; // 0x1e0, FCODE value
	v32 RET_DEC_PIC_TRD;       // 0x1e4, TRD value
	v32 RET_DEC_PIC_TIME_BASE_LAST;  // 0x1e8, TIME_BASE_LAST value
	v32 RET_DEC_PIC_NONB_TIME_LAST;  // 0x1ec, NONB_TIME_LAST value
	v32 RET_DEC_PIC_BCNT;  // 0x1f0, the size of frame consumed
	
} S3C6400_MFC_PARAM_REG_DEC_PIC_RUN;


typedef struct tagS3C6400_MFC_PARAM_REG_SET_FRAME_BUF {
	v32 SET_FRAME_BUF_NUM;
	v32 SET_FRAME_BUF_STRIDE;
...
typedef struct tagS3C6400_MFC_PARAM_REG_ENC_SEQ_INIT {
	...
	/* 0x180 */
	v32 ENC_SEQ_BIT_BUF_ADDR; // BitBufAddr. Bitstream buffer SDRAM byte address
	
	/* 0x184 */
	v32 ENC_SEQ_BIT_BUF_SIZE;  //BitBufSize. Bitstream buffer size in kilo bytes count

	/* 0x188 */
	struct {
		v32 MbBitReport         :1, // [0] Bit position of every MB is stored to SDRAM buffer
			SliceInfoReport     :1, // [1] Enable encoded Slice number and position in SDRAM bit
	...
	/* 0x18C */
	v32 ENC_SEQ_COD_STD;  // Encode Coding Standard. 0: MPEG4 Simple Profile, 1: MPEG4 Short Video Header / H.263+, 2: H.264

	/* 0x190 */
	struct {
		v32 PictureHeight      :10, // Encode source picture height size in pixel
			PictureWidth       :10; // Encode source picture width size in pixel
	} ENC_SEQ_SRC_SIZE;
	...
	struct {
		v32 DataPartEn          :1,  // [0] 0: Data Partition Disable, 1: Data Patition Enable
			RevVlcEn            :1,  // [1] 0: Normal VLC table used, 1: Reversible VLC table used
	...
	struct {
		v32 AnnexT              :1,  // [0] 0: Annex T off, 1: Annex T on
			...
			AnnexI              :1;  // [3] 0: Annex I off, 1: AnnexI on
	} ENC_SEQ_263_PARA;

	/* 0x1A0 */
	struct {
		v32 ChromaQpOffset      :5,  // [4:0]   chroma_qp_index_offset in Picture Parameter set range -12 to +12
			ConstIntraFlag      :1,  // [5]     constrained_intra_pred_flag in Picture Parameter set
			...
			DeblkAlphaOffset    :4,  // [11:8]  slice_alpha_c0_offset_div2 in slice header range -6 to +6
			DeblkBetaOffset     :4;  // [15:12] slice_beta_offset_div2 in slice header range -6 to +6
	} ENC_SEQ_AVC_PARA;
		
	/* 0x1A4 */
	struct {
		v32 SliceMode           :1,  // [0] 0: one slick per picture, 1: multiple slices per picture
			SliceSizeMode       :1,  // [1] 0: Slice is changed by encoded slice bit number. 1: Slice is changed by encoded macro-block number
			SlickSizeNum        :14; // [15:2] If SliceSizeMode is 0, macro-block number of one slice must be set to this register. If SliceSizeMode is 1, encoded bit count of one slice must be set to this register
	} ENC_SEQ_SLICE_MODE;

		/* 0x1A8 */
	v32 ENC_SEQ_GOP_NUM;  // Encode GOP number. I picture is inserted at every GOP picture number. Maximum GOP number is 60

	/* 0x1AC */
	struct {
		v32 RcEnable            :1,  // [0]     Rate Control Enable. 0: the value of register PictureQs is used as a Quantization Step in whole sequence
			BitRate             :15, // [15:1]  Target Bit Rate in kilo bit per seconds (kbps)
			InitDelay           :15, // [30:16] Reference Decoder initial buffer removal delay in millisecond (ms)
			SkipDisable         :1;  // Rate control automatic skip disable. 0: BIT processor may skip one picture if available bits are insufficient for accommodate the bit budget. 1: BIT processor never skip the picture but encoded bitstream may be overflow than target bit rate at hard-to-encode sequences
	} ENC_SEQ_RC_PARA;

	/* 0x1B0 */
	v32 ENC_SEQ_RC_BUF_SIZE; // Reference Decoder buffer size in bits

	/* 0x1B4 */
	v32 ENC_SEQ_INTRA_MB;   // Intra MB refresh number. Must be less than encoded (PictureHeight*PictureWidt/256). 0: Intra MB refresh is not used, N: At least N number of MBs are encoded as Intra mode at every picture
	...
	/* 0x1BC */
	v32 CMD_ENC_INTRA_QP;  // Intra frame picture quantized step parameter for encoding process	
	...
	/* 0x1D0 */
	v32 CMD_ENC_SEQ_TMP_BUF_1; // Temporary buffer SDRAM byte address. Temporary buffer 1 is used as ACDC prediction Buffer for Mpeg4 and Intra Prediction Y buffer for AVC

	/* 0x1D4 */
	v32 CMD_ENC_SEQ_TMP_BUF_2;  // Temporary buffer SDRAM byte address. Temporary buffer 2 is used as data partition part 2 save

	/* 0x1D8 */
	v32 CMD_ENC_SEQ_TMP_BUF_3; // Temporary buffer SDRAM byte address. Temporary buffer 3 is used as data partition part 3 save

	/* 0x1DC */
	v32 CMD_ENC_SEQ_TMP_BUF_4; // Temporary buffer SDRAM byte address. Temporary buffer 4 is used when FMO option is enabled in H.264 encoder
...
typedef struct tagS3C6400_MFC_PARAM_REG_DEC_SEQ_INIT {
	// INPUT ARGUMENTS common for rnc.dec
	// (0x180~ 0x1A0)
	v32 DEC_SEQ_BIT_BUF_ADDR;  // 0x180

	v32 DEC_SEQ_BIT_BUF_SIZE;  // 0x184
	...
	/* 0x18C */
	v32 DEC_SEQ_PRO_BUF;   // Process buffer SDRAM byte address. Process buffer must be 256 byte-aligned. Host must write this register before executing DEC_SEQ_INIT command

	/* 0x190 ~ 0x1A0*/
	v32 DEC_SEQ_TMP_BUF1; // Temporary buffer SDRAM byte address. Temporary buffer 1 is used as ACDC prediction buffer for mpeg4 and Intra prediction Y buffer for AVC and ACDC prediction buffer for VC1.
	v32 DEC_SEQ_TMP_BUF2; // Temporary buffer 2 is used as data partition part 1 save buffer for mpeg4 and Intra prediction Cb buffer for AVC and deblocking buffer for VC1
	v32 DEC_SEQ_TMP_BUF3; // Temporary buffer 3 is used as data partition part 2 save buffer for mpeg4 and Intra prediction Cr buffer for AVC and not used for VC1
	v32 DEC_SEQ_TMP_BUF4; // Temporary buffer 4 is used as slice information save buffer for AVC and not used for VC1 and mpeg4
	v32 DEC_SEQ_TMP_BUF5; // Temporary buffer 5 is used as slice information save buffer for AVC and not used for VC1 and mpeg4

	/* 0x1A4 */
	v32 DEC_SEQ_START_BYTE; // Byte Address of valid bitstream in input stream buffer
	...
	/* 0x1C8 */
	struct {
		v32 FrameRateRes       :16, // [15:0]  Decoded picture frame rate residual
			FrameRateDivMinux1 :16; // [31:16] Decoded picture frame rate unit number in Hz minus 1
	} RET_DEC_SEQ_SRC_FRAME_RATE;

    /* 0x1CC */
	v32 RET_DEC_SEQ_FRAME_NEED_COUNT; // Minimum decoded frame buffer need to decode stream successfully

	/* 0x1D0 */
	v32 RET_DEC_SEQ_FRAME_DELAY; // Maximum display frame buffer delay for buffering decoded picture reorder

	/* 0x1D4 */
	struct {
		v32 DataPartEn         :1,   // 0: Data Partition Disable, 1: Data Partition Enable
			RevVlcEn           :1,  // 0: Normal VLC table used, 1: Reversible VLC table used
			ShortVideoHeader   :1,  // 0: Normal MPEG4 Stream, 1: Short Video Header Stream
			H263AnnexJ         :1;  // 0: Annex J off, 1: Annex J On
	} RET_DEC_SEQ_INFO;

	v32 RET_DEC_SEQ_TIME_RES; // 0x1d8  datasheet has no explanation
	
} S3C6400_MFC_PARAM_REG_DEC_SEQ_INIT;


typedef struct {
	v32 BitStreamRdPtr; // 0x120  Bit Stream Buffer Read Address of Run Index 0
	v32 BitStreamWrPtr; // 0x124  Bit Stream Buffer Write Address of Run Index 0
...
typedef struct tagS3C6400_MFC_HOSTIF_REG {
	...
	struct {
		v32 CodeData            :16, // [15:0]  16-bit BIT code download data
			CodeAddr            :13; // [28:16] 13-bit BIT code download address
	} CodeDownLoad;  // Code Download Data register

	/* 0x008 */
	v32 HostIntReq; // Host Interrupt Request to BIT processor. Host can write '1' to this register to request interrupt to BIT
	...
	struct {
		v32 SelBigEndian            :1, // [0] 0: bit stream buffer is 4 byte little endian format, 1: bit stream buffer is big endian
			BufStsCheckDis          :1, // [1] 0: bit stream buffer overflow/underflow check enable, 1: ... disable
			BufPicFlush             :1, // [2] 1: bit stream buffer is flushed at every end of encoding picture
			BufPicReset             :1, // [3] 1: bit stream buffer is reset at every picture encoding/decoding command
			EncDynbufAllocEn        :1; // [4] Enable dynamic picture stream buffer allocation in encoder operations
	...
	/* 0x114 */
	v32 DecFuncCtrl;   // Decoder Function Control. [0]StreamEnd 0: There is more bitstream to be given to the decoder. 1: The whole bitstream has been given to the decoder
	...
	/* 0x160 */
	v32 BusyFlag;    // Processor Busy Flag. 0: BIT processor is ready for host command. 1: BIT processor is executing host command and not completed yet
	
	/* 0x164 */
	v32 RunCommand;  // Host writes the command code to this register

	/* 0x168 */
	v32 RunIndex;    // Run Process Index. Host writes the codec process index to this register before every writing run command

	/* 0x16C */
	v32 RunCodStd;   // Run Codec Standard. Host writes the codec standard index code to this register before every writing run command
	
	/* 0x170 */
	v32 IntEnable;   // Interrupt Enable Flag register. Each bit of this register is interrupt enable flag of various interrupt. 1: interrupt enable so BIT generates interrupt; 0: interrupt disable 0th bit (LSB),Initialize complete
	
	/* 0x174 */
	v32 IntReason;   // Interrupt Reason Flag register. Each bit of this register is interrupt report flag of each interrupt. 1: interrupt is generated, 0: not generated

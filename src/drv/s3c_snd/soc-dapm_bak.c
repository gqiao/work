	...
	for (i = 0; i < w->num_kcontrols; i++) {
		...
		list_for_each_entry(path, &w->sources, list_sink) {
			...
			if ((ret = snd_ctl_add(codec->card, path->kcontrol)) < 0) {
				__E("asoc: failed to add dapm kcontrol %s\n",
						path->long_name);
...
static int is_connected_output_ep(struct snd_soc_dapm_widget *widget)
{
	...
	if (widget->connected) {
		...
		if (widget->id == snd_soc_dapm_hp || widget->id == snd_soc_dapm_spk || widget->id == snd_soc_dapm_line)
			return 1;
	}
...
static int dapm_power_widgets(struct snd_soc_codec *codec, int event)
{
	...
	for(i = 0; i < c; i++) {
		list_for_each_entry(w, &codec->dapm_widgets, list) {
			...
			if (power_change) {
				if (w->event) {
					__D("power %s event for %s flags %x\n",
		...
		case snd_soc_dapm_mixer:
			if (w->name) {
				...
				list_for_each_entry(p, &w->sources, list_sink) {
					if (p->connect)
						__D(" in  %s %s\n", p->name ? p->name : "static", p->source->name);
				}
				 ...
				 int val, struct soc_enum* e)
{
	...
	if (snd_soc_test_bits(widget->codec, e->reg, mask, val) != 0)
		return 0;
	...
	list_for_each_entry(path, &widget->codec->dapm_paths, list) {
		...
		if (path->name == NULL || e->texts[val] == NULL)
			continue;
...
err:
	__E("asoc: no dapm match for %s --> %s --> %s\n", source,
		control, sink);
	kfree(path);
	return ret;
}
			    ...
			    const struct snd_soc_dapm_route *route, int num)
{
	...
	for (i = 0; i < num; i++) {
		if ((ret = snd_soc_dapm_connect_input(codec, route->sink,
											 route->control, route->source)) < 0) {
			__E("Failed to add route %s->%s\n", route->source, route->sink);
			return ret;
		}
		route++;
	}
	...
	list_for_each_entry(w, &codec->dapm_widgets, list)
	{
		...
		case snd_soc_dapm_mux:
			...
			if (w->num_kcontrols == 0) {
				__E("asoc: mux %s has no controls\n", w->name);
				break;
			}
			kcontrol = snd_soc_cnew(&w->kcontrols[0], w, w->name);
			if ((ret = snd_ctl_add(codec->card, kcontrol)) < 0) {
				__E("asoc: failed to add kcontrol %s\n", w->name);
				break;
			}
	...
	int num)
{
	...
	for (i = 0; i < num; i++) {
		if ((ret = snd_soc_dapm_new_control(codec, widget)) < 0)
			return ret;
		widget++;
	}
	return 0;

	...
	unsigned int value)
{
	...
	__I(AUDIO_NAME ": 0x%02x%02x, R%02d <= 0x%03x\n",
...
static inline int get_coeff(int mclk, int rate)
{
	...
	__E("wm8987: could not get coeff for mclk %d @ rate %d\n", mclk, rate);
	return -EINVAL;
}
...
static int wm8987_init(struct snd_soc_device *socdev)
{
	...
	if (ret < 0) {
		__E("wm8987: failed to create pcms\n");
		goto pcm_err;
	}
	...
	wm8987_write(codec, WM8987_ADCDAC, reg |  0x004);	//HHTECH wk
//	wm8987_write(codec, WM8987_PWR2, 0x180);	// reboot
	...
	wm8987_write(codec, WM8987_ADCTL2, reg |  0x0004);	// XXX:	    //HHTECH wk
	...
	if (ret < 0) {
		__E("wm8987: failed to register card\n");
		goto card_err;
	}
...
static int wm8987_codec_probe(struct i2c_adapter *adap, int addr, int kind)
{
	...
	if (ret < 0) {
	__E("failed to initialise WM8987\n");
		goto err;
	}
...
static int wm8987_probe(struct platform_device *pdev)
{
	...
	__D("\n");
	__I("Audio Codec Driver %s", WM8987_VERSION);

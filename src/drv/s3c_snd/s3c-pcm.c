...
static void s3c24xx_pcm_enqueue(struct snd_pcm_substream *substream)
{
	...
	while (prtd->dma_loaded < prtd->dma_limit) {
		...
		if ((pos + len) > prtd->dma_end) {
			...
			__D("corrected dma len %ld\n", len);
		}

		if ((ret = s3c2410_dma_enqueue(prtd->params->channel, 
									   substream, pos, len)) == 0) {
			prtd->dma_loaded++;
			pos += prtd->dma_period;
			if (pos >= prtd->dma_end)
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (prtd->params == NULL) {
		prtd->params = dma;
		__D("params %p, client %p, channel %d\n", prtd->params,
			prtd->params->client, prtd->params->channel);

		/* prepare DMA */
		if ((ret = s3c2410_dma_request(prtd->params->channel,
									   prtd->params->client, NULL)) != 0) {
			__E("failed to get dma channel\n");
			return ret;
		}
...
static int s3c24xx_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (prtd)
		...
		__E("s3c24xx_pcm_close called with prtd == NULL\n");
...
static int s3c24xx_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	if ((buf->area = dma_alloc_writecombine(pcm->card->dev, size,
											&buf->addr, GFP_KERNEL)) == NULL) {
		__D("dma_alloc_writecombine() error\n");
		return -ENOMEM;
	}
	buf->bytes = size;
	return 0;
...
static void s3c24xx_pcm_free_dma_buffers(struct snd_pcm *pcm)
{
	...
	for (stream = 0; stream < 2; stream++) {
		if ((substream = pcm->streams[stream].substream) == NULL)
			continue;
						   ...
						   struct snd_soc_codec_dai *dai, struct snd_pcm *pcm)
{
	...
	if (dai->playback.channels_min) {
		if ((ret = s3c24xx_pcm_preallocate_dma_buffer(pcm,

...
#define SAA7113H_VERSION_ID_REG 0
...
camif_cis_t msdma_input = {
	.ITU            = CAMIF_ITU601,
	.order422_CAM   = CAMIF_YCBYCR,		/* YCRYCB */
...
camif_cis_t interlace_input = {
	.ITU            = CAMIF_ITU656,
	.order422_CAM   = CAMIF_CBYCRY,		/* YCRYCB */
	.camclk         = 32000000, 		/* No effect */
	.source_x       = 720,       
	.source_y       = 290,
	.win_hor_ofst   = 16,
...
{
	__D("\n");
	if(interlace_input.init_sensor == 0)
		return NULL;
	return &interlace_input;
}
EXPORT_SYMBOL(get_initialized_cis);
#endif

static unsigned short normal_addr[] = {0x4a >> 1, I2C_CLIENT_END };
...
{   __D("\n");
//	printk("saa711x_write reg 0x%x value 0x%x\n",reg,value);
...
static const unsigned char saa7115_init_misc[] = {
	...
	R_02_INPUT_CNTL_1, 0xC0,		/* lzcx input tuner -> input 1, amplifier active */
...
static int saa711x_attach(struct i2c_adapter *adapter, int address, int kind)
{
	...
	snprintf(client->name, sizeof(client->name) - 1, "saa711x");
#if 1
	if (kind < 0) {
		/* Get chip version */
		...
		if (version == -1){
			printk("i2c err at addr 0x%x\n",address);
			return -1;
		}

		/* If not a SAA7113H, bail out */
		if ((version & 0xf0) != SAA7113H_I2C_VERSION_ID) {
			printk ("unknown SAA711x chip found, chip version: %#x\n", version);
			return -1;
		}
...
#if 1 	
		printk ("video decoder chip attached,"
				" rev 0x%02x at %#02x\n", version,
				(client->addr << 1));
#endif	
	}
	else {
		printk ("detection skipped\n");
	}
#endif
	...
	printk("SAA711%d found (%s) @ 0x%x (%s)\n", chip_id, name, address << 1, adapter->name);

	state = kzalloc(sizeof(struct saa711x_state), GFP_KERNEL);
	i2c_set_clientdata(client, state);
	if (state == NULL) {
		printk("state is NULL\n");
		kfree(client);
		return -ENOMEM;
	}
...
//		v4l_info(client, "WARNING: Chip is not known - Falling back to saa7111\n");
	...
	saa711x_set_v4lstd(client, V4L2_STD_PAL); //lzcx
#if 1
	...
	printk("SAA7113 driver OK!\n");
	return 0;
}

static int saa711x_probe(struct i2c_adapter *adap)
{   __D("\n");
	return i2c_probe(adap, &addr_data, saa711x_attach);
}

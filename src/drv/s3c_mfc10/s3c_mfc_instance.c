#define LOG_TAG "s3c_mfc_instance.c"
#include "osal.h"
...
static s3c_mfc_inst_context_t _mfcinst_ctx[MFC_NUM_INSTANCES_MAX];
...
{   __D("\n");
	ctx->stream_buffer = (unsigned char *)(GetDataBufVirAddr() + \
				(ctx->inst_no * MFC_LINE_BUF_SIZE_PER_INSTANCE));
	ctx->phys_addr_stream_buffer = (unsigned int)(s3c_mfc_get_databuf_phys_addr() +	\
					(ctx->inst_no * MFC_LINE_BUF_SIZE_PER_INSTANCE));
	ctx->stream_buffer_size = MFC_LINE_BUF_SIZE_PER_INSTANCE;
...
{   __D("\n");
	...
	ctx->phys_addr_yuv_buffer = S3C6400_BASEADDR_MFC_DATA_BUF +					\
				((int)instance_yuv_buffer - (int)GetDataBufVirAddr());
...
{   __D("\n");
	...
	writel(MFC_LINE_BUF_SIZE_PER_INSTANCE / 1024, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_DEC_SEQ_BIT_BUF_SIZE);
...
{   __D("\n");
	...
	switch(ctx->inst_no) {
	case 0:
		writel(ctx->phys_addr_stream_buffer + MFC_LINE_BUF_SIZE_PER_INSTANCE, 		\
	...
	writel(MFC_LINE_BUF_SIZE_PER_INSTANCE / 1024, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_BIT_BUF_SIZE);

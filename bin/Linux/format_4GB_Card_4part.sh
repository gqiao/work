...
names=(   "boot"    "rootfs"  "rootfsp3" "rootfsp5")
types=(   "0x0C,*"  "L"         "L"            "L" )
formats=( "vfat"    "ext2"    "ext2"         "ext2")
sizes=(     307       60       60             "" )
...
CYLINDERS=`echo ${SIZE_B}/${HEADS}/${SECTORS}/512 | bc`
...
} | simpcmd "Partition" sudo sfdisk -D -H ${HEADS} -S ${SECTORS} -C $CYLINDERS $DRIVE
sleep 1s
...
for (( i=0; i<${#names[*]}; i++ )); do
    ...
    printf "%-16s%-16s %-8s %3s cyl %4d MB %4d MiB\n"                 \
